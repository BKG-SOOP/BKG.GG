...
v5 반영 내용
- 카드 내부를 프로필 이미지 중심 레이아웃으로 재정리
- 프로필 이미지/BKG 배지를 148px 중심 요소로 확대
- 누적 전적/월별 전적/참여율/최근 5전 박스를 최소 공간형으로 축소
- 참여율과 최근 5전 박스에 compact 스타일 적용
- 최근 5전은 5개 고정 한 줄 표시
- 전체 카드/컨테이너 폭을 확대해 3장 비교 화면 가독성 개선


v5.1 반영 내용
- 관리자 로그인/로그아웃 버튼 스타일 복구 및 개선
- 새로고침 버튼 스타일 복구 및 개선
- 멤버 수정 버튼을 작은 다크톤 pill 버튼으로 정리
- 모달 저장/취소/닫기 버튼 스타일 정돈


v5.2 반영 내용
- BKG-SOOP 전적표 스타일에 맞춰 관리자/새로고침/멤버수정 버튼을 다크 블루 pill 스타일로 재정리
- 기존 CSS와 브라우저 기본 버튼 스타일을 확실히 덮어쓰기 위한 최종 override 추가


v5.3 반영 내용
- BKG.GG 티어 옵션에 0티어 / 최하 추가
- 0티어 색상은 기존 #b4a7d6 유지
- 기존 전적표에서 0티어 최하로 이동된 멤버도 BKG.GG에서 정상 표시/수정 가능
